...
import itertools # For the spinner
...
class HUCProcessor:
    ...
    def _process_single_huc(self, huc8_id: str):
        try:
            ...
            # --- 1. Export HUC Boundary ---
            print(f"  Creating background task to export HUC boundary for {huc8_id}...")
            desc_boundary = f'Boundary_Export_HUC_{huc8_id}'
            boundary_task = ee.batch.Export.table.toDrive(
                collection=ee.FeatureCollection([huc_feature]),
                description=desc_boundary,
                folder=gdrive_folder_name,
                fileNamePrefix=f'huc8_{huc8_id}_boundary',
                fileFormat='GeoJSON'
            )
            self.all_tasks.append({'task': boundary_task, 'description': desc_boundary})
            
            # --- 2. Export Full HUC DEM ---
            ...
            # --- 3. Generate and Save Center Points ---
            ...
            if num_centers > 0:
                ...
                # --- 3a. Export Center Point Features to Google Drive ---
                print(f"  Creating background task to export center point features for HUC {huc8_id}...")
                desc_centers_features = f'Center_Points_Features_Export_HUC_{huc8_id}'
                centers_features_task = ee.batch.Export.table.toDrive(
                    collection=center_points_with_coords_fc, 
                    description=desc_centers_features,
                    folder=gdrive_folder_name, 
                    fileNamePrefix=f'huc8_{huc8_id}_center_points_features',
                    fileFormat='GeoJSON'
                )
                self.all_tasks.append({'task': centers_features_task, 'description': desc_centers_features})

                # --- 3b. Fetch point data to save locally as CSV ---
