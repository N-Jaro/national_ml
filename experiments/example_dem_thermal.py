#!/usr/bin/env python3
"""
Example script demonstrating how to use the DEM + Thermal model.
...
def load_model(checkpoint_path: str, device: torch.device) -> MultimodalMultitaskModel_DEM_Thermal:
...
def run_inference_example(data_path: str, huc_codes: list, model_path: str = None):
    ...
    with torch.no_grad():
        for i in range(num_samples):
            ...
            print(f"Thermal stats: min={thermal.min():.3f}, max={thermal.max():.3f}, mean={thermal.mean():.3f}")
...
def show_data_compatibility():
    """
    Show how to adapt existing data loading code to work with the DEM + Thermal model.
    """
    print("\\n" + "="*60)
    ...
    print("\\nNEW (DEM + Thermal) data loading:")
    print("""
    batch = dataset[0]  # Returns dict with keys:
    # - 'dem': DEM (1, H, W)  
    # - 'thermal': Thermal (1, H, W)
    # - 'hydro_mask': (H, W)
    # - 'flow_dir': (H, W)
    ...
    print("1. Replace MultimodalPatchDataset with MultimodalPatchDataset_DEM_Thermal")
    print("2. Change model forward call from model(m1, m2, m3, m4) to model(dem, thermal)")
    print("3. Update data dictionary key access from 'm1', 'm3' to 'dem', 'thermal'")
    
    print("\\nModel differences:")
    print("- Reduced from 4 encoders to 2 encoders (DEM + Thermal)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DEM + Thermal Model Example')
    ...
    if args.data_path and os.path.exists(args.data_path):
        ...
        print("Example: python example_dem_thermal.py --data_path /path/to/your/data --huc_codes 10020007")
