...
class MultimodalMultitaskModel_DEM_SAR(nn.Module):
    def __init__(self, n_classes_task1=1, n_classes_task2=1, base_channels=64):
        super().__init__()
        
        # 1. Separate Encoders for DEM and SAR modalities only
        self.encoder_dem = ModalityEncoder(in_channels=1, base_channels=base_channels) # DEM (Priority modality)
        self.encoder_sar = ModalityEncoder(in_channels=1, base_channels=base_channels) # SAR
        ...
        self.hierarchical_fusion = HierarchicalAttentionFusion(
            ...
            other_channels_list=[encoder_out_channels], # Only SAR as secondary modality
    ...
    def forward(self, dem, sar):
        ...
        # f_dem is the primary feature, [f_sar] is the secondary
        fused_features = self.hierarchical_fusion(f_dem, [f_sar])
...
if __name__ == '__main__':
    ...
    sar = torch.randn(batch_size, 1, 224, 224).to(device)  # SAR data
    ...
    try:
        output_task1, output_task2 = model(dem, sar)
        ...
        print("\nModel forward pass successful with DEM + SAR architecture!")
