...
class AlphaEarthEncoder(nn.Module):
    ...
    AlphaEarth provides high-dimensional embeddings from Google's satellite embedding model.
...
class TaskDecoder(nn.Module):
    ...
    def __init__(self, n_classes, base_channels=64, bilinear=True):
        super().__init__()
        # Channel counts are now based on a single set of skip connections, not five.
...
class MultimodalMultitaskModel_All_Modalities_AlphaEarth(nn.Module):
    """
    Comprehensive MDMT model combining all modalities + AlphaEarth for multitask learning.
    
    Combines:
    - DEM (Digital Elevation Model) - topographic information - priority modality
    - Optical (6-band Landsat) - visible/NIR/SWIR spectral data  
    - Thermal (1-band Landsat) - thermal infrared data
    - SAR (1-band Sentinel-1) - synthetic aperture radar data
    - AlphaEarth (64-band embeddings) - Google's satellite foundation model embeddings
    
    Total input channels: 1 + 6 + 1 + 1 + 64 = 73 channels
    
    This fusion provides the most comprehensive representation combining:
    - Explicit topographic information (DEM)
    - Rich spectral information across optical/thermal spectrum (Landsat)
    - Weather-independent SAR information (Sentinel-1)  
    - Foundation model contextual patterns (AlphaEarth)
    """
    def __init__(self, n_classes_task1=1, n_classes_task2=8, base_channels=64):
        super().__init__()
        
        # 1. Separate Encoders for all modalities
        self.encoder_dem = ModalityEncoder(in_channels=1, base_channels=base_channels)      # DEM (Priority)
        self.encoder_optical = ModalityEncoder(in_channels=6, base_channels=base_channels) # Landsat optical
        self.encoder_thermal = ModalityEncoder(in_channels=1, base_channels=base_channels) # Landsat thermal
        self.encoder_sar = ModalityEncoder(in_channels=1, base_channels=base_channels)     # Sentinel-1 SAR
        self.encoder_alphaearth = AlphaEarthEncoder(in_channels=64, base_channels=base_channels) # AlphaEarth (fixed 64 channels)
        
        # 2. Hierarchical Attention Fusion (DEM as primary, others as secondary)
        ...
        self.hierarchical_fusion = HierarchicalAttentionFusion(
            ...
            other_channels_list=[encoder_out_channels] * 4, # Optical, Thermal, SAR, AlphaEarth as secondary
            fused_channels=encoder_out_channels
        )
        
        # 3. Shared Deep Encoder (more processing for comprehensive fusion)
        self.shared_encoder = nn.Sequential(
            DoubleConv(encoder_out_channels, encoder_out_channels),
            # Additional processing for the rich fusion of all modalities
            DoubleConv(encoder_out_channels, encoder_out_channels),
            # Extra depth for complex multi-modal integration
            DoubleConv(encoder_out_channels, encoder_out_channels),
        )
    ...
    def forward(self, dem, optical, thermal, sar, alphaearth):
        # Pass each modality through its encoder
        f_dem, dem_skips = self.encoder_dem(dem)                          # DEM as priority (features and skips)
        f_optical, _ = self.encoder_optical(optical)                      # Optical features (skips ignored)
        f_thermal, _ = self.encoder_thermal(thermal)                      # Thermal features (skips ignored)
        f_sar, _ = self.encoder_sar(sar)                                  # SAR features (skips ignored)
        ...
        # f_dem is the primary feature, others are secondary
        fused_features = self.hierarchical_fusion(f_dem, [f_optical, f_thermal, f_sar, f_alphaearth])
        
        # Pass fused features through the shared encoder (deeper processing for comprehensive fusion)
...
if __name__ == '__main__':
    ...
    batch_size = 2
    dem = torch.randn(batch_size, 1, 224, 224).to(device)           # DEM
    optical = torch.randn(batch_size, 6, 224, 224).to(device)      # Landsat B2-B7
    thermal = torch.randn(batch_size, 1, 224, 224).to(device)      # Landsat B10
    sar = torch.randn(batch_size, 1, 224, 224).to(device)          # Sentinel-1
    alphaearth = torch.randn(batch_size, 64, 224, 224).to(device)  # AlphaEarth embeddings

    print("\n--- Input Shapes ---")
    print(f"DEM (Priority): {dem.shape}")
    print(f"Optical (Landsat B2-B7): {optical.shape}")
    print(f"Thermal (Landsat B10): {thermal.shape}")
    print(f"SAR (Sentinel-1): {sar.shape}")
    print(f"AlphaEarth: {alphaearth.shape}")
    print(f"Total input channels: {1 + 6 + 1 + 1 + 64} = 73")

    try:
        output_task1, output_task2 = model(dem, optical, thermal, sar, alphaearth)
        ...
        print("\nModel forward pass successful with comprehensive All Modalities + AlphaEarth architecture!")
    ...
    print("\nComprehensive All Modalities + AlphaEarth model combines:")
    print("- Topographic precision (DEM)")
    print("- Spectral richness (Landsat Optical + Thermal)")  
    print("- Weather-independent information (SAR)")
    print("- Foundation model contextual patterns (AlphaEarth 64 channels)")
    print("This is the most comprehensive MDMT variant!")