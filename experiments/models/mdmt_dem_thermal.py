...
class MultimodalMultitaskModel_DEM_Thermal(nn.Module):
    def __init__(self, n_classes_task1=1, n_classes_task2=1, base_channels=64):
        super().__init__()
        
        # 1. Separate Encoders for DEM and Thermal modalities only
        self.encoder_dem = ModalityEncoder(in_channels=1, base_channels=base_channels) # DEM (Priority modality)
        self.encoder_thermal = ModalityEncoder(in_channels=1, base_channels=base_channels) # Thermal
        ...
        self.hierarchical_fusion = HierarchicalAttentionFusion(
            ...
            other_channels_list=[encoder_out_channels], # Only thermal as secondary modality
    ...
    def forward(self, dem, thermal):
        ...
        # f_dem is the primary feature, [f_thermal] is the secondary
        fused_features = self.hierarchical_fusion(f_dem, [f_thermal])
...
if __name__ == '__main__':
    ...
    model = MultimodalMultitaskModel_DEM_Thermal(n_classes_task1=1, n_classes_task2=1).to(device)

    batch_size = 2
    dem = torch.randn(batch_size, 1, 224, 224).to(device)
    thermal = torch.randn(batch_size, 1, 224, 224).to(device)
    ...
    try:
        output_task1, output_task2 = model(dem, thermal)
        ...
        print("\nModel forward pass successful with DEM + Thermal architecture!")
