...
#-----------------------------------------------------------------------------
# Slurm job directives for Segmentation-Only DEM + AlphaEarth model training (single job).
# Following the exact same pattern as submit_train_dem_alphaearth_single.sh
#-----------------------------------------------------------------------------

# Set the name of the job
#SBATCH --job-name=mdmt-segonly-dem-alphaearth-single
...
# Set the memory for the job (production config: batch_size=32, full precision)
#SBATCH --mem=180G

# Set the maximum time the job will run (500 epochs with early stopping)
#SBATCH --time=72:00:00

# Specify the output and error files for the job
#SBATCH --output=slurm_logs_segmentation_training/mdmt-segonly-dem-alphaearth-single_%j.out
#SBATCH --error=slurm_logs_segmentation_training/mdmt-segonly-dem-alphaearth-single_%j.err
...
WANDB_NAME="segonly_dem_alphaearth_${DATE_TIME}_run1"

# Create the log directory if it doesn't exist
mkdir -p slurm_logs_segmentation_training

echo "======================================================"
echo "Starting single Segmentation-Only DEM + AlphaEarth Training job"
...
echo "======================================================"
echo "Segmentation-Only DEM + AlphaEarth Training job completed"
