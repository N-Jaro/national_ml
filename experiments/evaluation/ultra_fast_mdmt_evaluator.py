#!/usr/bin/env python3
"""
ULTRA-FAST MDMT Evaluator - Unified evaluator for all MDMT variants.
This version supports all 6 variants with automatic variant detection and lazy loading.
"""
...
from mdmt_dem_thermal import MultimodalMultitaskModel_DEM_Thermal
...
from patchDataLoader_alphaearth_only_fast import MultimodalPatchDataset_AlphaEarth_Only_Fast as MultimodalPatchDataset_AlphaEarth_Only
from patchDataLoader_dem_only_fast import MultimodalPatchDataset_DEM_Only_Fast as MultimodalPatchDataset_DEM_Only
from patchDataLoader_dem_thermal_fast import MultimodalPatchDataset_DEM_Thermal_Fast as MultimodalPatchDataset_DEM_Thermal
from patchDataLoader_dem_sar_fast import MultimodalPatchDataset_DEM_SAR_Fast as MultimodalPatchDataset_DEM_SAR
from patchDataLoader_dem_optical_fast import MultimodalPatchDataset_DEM_OPTICAL_Fast as MultimodalPatchDataset_DEM_Optical
from patchDataLoader_dem_alphaearth_fast import MultimodalPatchDataset_DEM_ALPHAEARTH_Fast as MultimodalPatchDataset_DEM_AlphaEarth
...
VARIANT_CONFIG = {
    'alphaearth': {
        'model_class': MultitaskModel_AlphaEarth_Only,
        'dataset_class': MultimodalPatchDataset_AlphaEarth_Only,
        'model_params': {
            'n_classes_task1': 1,
            'n_classes_task2': 8,
            'base_channels': 64,
    ...
    'dem_only': {
        'model_class': MultitaskModel_DEM_Only,
        'dataset_class': MultimodalPatchDataset_DEM_Only,
        'model_params': {
            'n_classes_task1': 1,
            'n_classes_task2': 8,
            'base_channels': 64
    ...
    'dem_thermal': {
        'model_class': MultimodalMultitaskModel_DEM_Thermal,
        'dataset_class': MultimodalPatchDataset_DEM_Thermal,
        'model_params': {
            'n_classes_task1': 1,
            'n_classes_task2': 8,
            'base_channels': 64
    ...
    'dem_sar': {
        'model_class': MultimodalMultitaskModel_DEM_SAR,
        'dataset_class': MultimodalPatchDataset_DEM_SAR,
        'model_params': {
            'n_classes_task1': 1,
            'n_classes_task2': 8,
            'base_channels': 64
    ...
    'dem_optical': {
        'model_class': MultimodalMultitaskModel_DEM_Optical,
        'dataset_class': MultimodalPatchDataset_DEM_Optical,
        'model_params': {
            'n_classes_task1': 1,
            'n_classes_task2': 8,
            'base_channels': 64
    ...
    'dem_alphaearth': {
        'model_class': MultimodalMultitaskModel_DEM_AlphaEarth,
        'dataset_class': MultimodalPatchDataset_DEM_AlphaEarth,
        'model_params': {
            'n_classes_task1': 1,
            'n_classes_task2': 8,
            'base_channels': 64,
    ...
    'landsat6b': {
        'model_class': MultimodalMultitaskModel_ls6b,
        'dataset_class': MultimodalPatchDataset,
        'model_params': {
            'n_classes_task1': 1,
            'n_classes_task2': 8,
            'base_channels': 64
...
def compute_metrics_original(water_pred: torch.Tensor, water_target: torch.Tensor,
    ...
    """Compute comprehensive evaluation metrics - EXACT COPY from original evaluators"""
    metrics = {}
...
def fast_evaluate_huc(model: torch.nn.Module, huc_id: str, variant: str,
                      test_data_path: str, device: str = "cpu", 
                      limit_batches: int = None) -> Dict[str, Any]:
    """Fast evaluation for a single HUC"""
    
    logger.info(f"Evaluating HUC {huc_id} with {variant} variant...")
    ...
    # Create dataset with variant-specific parameters
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Evaluating HUC {huc_id}")):
            ...
            # Extract inputs based on variant
            inputs = []
            if variant == 'alphaearth':
                inputs = [batch['alphaearth'].to(device, non_blocking=True)]
            elif variant == 'dem_only':
                inputs = [batch['dem'].to(device, non_blocking=True)]
            elif variant == 'dem_thermal':
                inputs = [
                    batch['dem'].to(device, non_blocking=True),
                    batch['thermal'].to(device, non_blocking=True)
                ]
            elif variant == 'dem_sar':
                inputs = [
                    batch['dem'].to(device, non_blocking=True),
                    batch['sar'].to(device, non_blocking=True)
                ]
            elif variant == 'dem_optical':
                inputs = [
                    batch['dem'].to(device, non_blocking=True),
                    batch['optical'].to(device, non_blocking=True)
                ]
            elif variant == 'dem_alphaearth':
                inputs = [
                    batch['dem'].to(device, non_blocking=True),
                    batch['alphaearth'].to(device, non_blocking=True)
                ]
            elif variant == 'landsat6b':
                inputs = [batch['landsat'].to(device, non_blocking=True)]
            
            water_mask = batch['hydro_mask'].to(device, non_blocking=True)
            flow_dir = batch['flow_dir'].to(device, non_blocking=True)
            
            # Forward pass
            water_pred, d8_pred = model(*inputs)
            
            # Collect predictions and targets as TENSORS
...
def fast_evaluate_all_hucs(checkpoint_path: str, variant: str, huc_codes: List[str], 
                          test_data_path: str = "/projects/bcrm/nathanj/data/processed/test/patch_dataset",
                          device: str = "cpu", limit_batches: int = None) -> List[Dict[str, Any]]:
    """Ultra-fast evaluation function that evaluates each HUC separately"""
    
    if variant not in VARIANT_CONFIG:
        raise ValueError(f"Unsupported variant: {variant}. Supported variants: {list(VARIANT_CONFIG.keys())}")
    ...
    # Load checkpoint EXACTLY like original evaluators
    ...
    for huc_id in huc_codes:
        try:
            huc_metrics = fast_evaluate_huc(model, huc_id, variant, test_data_path, device, limit_batches)
            if huc_metrics:
                result = {
                    "timestamp": datetime.now().isoformat(),
                    "model_variant": f"mdmt_{variant}",
...
def main():
    parser = argparse.ArgumentParser(description="Ultra-fast MDMT Evaluator")
    ...
    # Run per-HUC evaluation
    if device.startswith('cuda'):
        torch.cuda.synchronize()
    start_time = datetime.now()
    results = fast_evaluate_all_hucs(args.checkpoint, args.variant, huc_codes, device=device, limit_batches=args.limit_batches)
    if device.startswith('cuda'):
        torch.cuda.synchronize()
    ...
    # Print results
    print("\n" + "="*80)
    print(f"ULTRA-FAST {args.variant.upper()} EVALUATION COMPLETED (PER-HUC)")
    print("="*80)
    ...
    csv_path = os.path.join(output_dir, f"{args.variant}_{run_name}_{timestamp}.csv")
