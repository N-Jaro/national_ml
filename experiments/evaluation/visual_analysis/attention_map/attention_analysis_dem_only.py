#!/usr/bin/env python3
"""
Multi-Layer Attention Analysis for DEM-Only Model
...
def normalize_dem(dem, stats):
    """Normalize DEM data using HUC-specific statistics."""
    if stats is None or 'dem' not in stats:
        print("⚠️  Warning: Using raw DEM data (no normalization stats)")
        return dem
    
    dem_mean = stats['dem']['elevation_mean']
    dem_std = stats['dem']['elevation_stdDev']
    return _zscore(dem, dem_mean, dem_std)
...
def get_gradients_manually(model, input_tensor, target_layer):
    ...
    try:
        # Forward pass
        model.zero_grad()
        output1, output2 = model(input_tensor)  # DEM-only model takes single input
...
def generate_multi_layer_attention_maps():
    """Generate attention maps across multiple model layers for DEM-only variant."""
    
    device = torch.device('cpu')
    print(f"🎯 Multi-Layer Attention Analysis - DEM-Only Model")
    print("=" * 60)
    ...
    # Load and prepare data (DEM-only uses just DEM)
    try:
        ...
        # Normalize DEM using HUC-specific statistics
        dem = normalize_dem(dem, stats)
        
        # Create input tensor for DEM-only model (single channel DEM)
        input_data = dem[np.newaxis, :, :].astype(np.float32)  # Add channel dimension: (1, 224, 224)
    ...
    # Load model
    print(f"📦 Loading DEM-only model...")
    model = MultitaskModel_DEM_Only(n_classes_task1=1, n_classes_task2=8)
    
    # Note: You'll need to update this path to the actual DEM-only checkpoint
    checkpoint_path = '/u/nathanj/national_ml/experiments/training/lightning_logs/dem_only_checkpoint.ckpt'
    ...
    try:
        if os.path.exists(checkpoint_path):
            ...
            print("   Please update the checkpoint path for DEM-only model")
            return
    except Exception as e:
    ...
    # Define layers to analyze for DEM-only architecture
    layer_configs = {
    ...
    output_file = output_dir / f'multi_layer_attention_dem_only_{huc_id}_{patch_filename}_{timestamp}.png'
