#!/usr/bin/env python3
"""
Simple Landsat 6B Model Evaluator (DEM + Optical(6) + SAR + Thermal)
...
def load_model_direct(checkpoint_path: str, device: str = "cpu") -> torch.nn.Module:
    ...
    try:
        # Create model instance (4-modality model with 6 optical bands)
        ...
        logger.info(f"Successfully loaded Landsat 6B model from {checkpoint_path}")
        return model
...
def evaluate_huc(model: torch.nn.Module, huc_id: str, test_data_path: str, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Evaluating HUC {huc_id}")):
            ...
            # Move data to device (4-modality inputs)
            m1 = batch['m1'].to(device)  # DEM  
            m2 = batch['m2'].to(device)  # Optical (6 bands)
            m3 = batch['m3'].to(device)  # Thermal
            m4 = batch['m4'].to(device)  # SAR
...
def main():
    parser = argparse.ArgumentParser(description="Simple Landsat 6B Model Evaluator")
    ...
    parser.add_argument("--output-dir", type=str,
                       default="/u/nathanj/national_ml/experiments/evaluation/simple_landsat6b_results",
    ...
    csv_path = output_dir / f"simple_landsat6b_evaluation_{timestamp}.csv"
    results_df.to_csv(csv_path, index=False)
    
    # Save JSON
    json_path = output_dir / f"simple_landsat6b_evaluation_{timestamp}.json"
    results_df.to_json(json_path, orient='records', indent=2)
    
    # Print summary
    print("\n" + "="*80)
    print("DEM + OPTICAL + THERMAL + SAR EVALUATION COMPLETED")
    print("="*80)
