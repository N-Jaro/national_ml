#!/usr/bin/env python3
"""
Simple DEM + Thermal Model Evaluator
...
def load_model_direct(checkpoint_path: str, device: str = "cpu") -> torch.nn.Module:
    ...
    try:
        ...
        logger.info(f"Successfully loaded DEM + Thermal model from {checkpoint_path}")
        return model
...
def main():
    parser = argparse.ArgumentParser(description="Simple DEM + Thermal Model Evaluator")
    ...
    parser.add_argument("--output-dir", type=str,
                       default="/u/nathanj/national_ml/experiments/evaluation/simple_dem_thermal_results",
    ...
    csv_path = output_dir / f"simple_dem_thermal_evaluation_{timestamp}.csv"
    results_df.to_csv(csv_path, index=False)
    
    # Save JSON
    json_path = output_dir / f"simple_dem_thermal_evaluation_{timestamp}.json"
    results_df.to_json(json_path, orient='records', indent=2)
    
    # Print summary
    print("\n" + "="*80)
    print("DEM + THERMAL EVALUATION COMPLETED")
    print("="*80)
