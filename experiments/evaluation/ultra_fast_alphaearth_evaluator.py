#!/usr/bin/env python3
"""
ULTRA-FAST AlphaEarth Evaluator using lazy loading approach.
...
from patchDataLoader_alphaearth_only_fast import MultimodalPatchDataset_AlphaEarth_Only
...
def setup_wandb(run_name: str, checkpoint_path: str, csv_path: str = None):
    ...
    # Initialize wandb
    wandb.init(
        project="mdmt_alphaearth_evaluation",
        name=run_name,
        config=run_info,
        tags=["alphaearth_only", "evaluation", "ultra_fast"]
    )
...
class FastAlphaEarthDataset(Dataset):
    """Ultra-fast dataset that loads and validates data on-demand"""
    
    def __init__(self, base_path: str, huc_codes: List[str]):
        self.base_path = base_path
        self.huc_codes = huc_codes
        
        # FAST: Just collect all possible files without validation
        self.all_files = []
        for huc_code in huc_codes:
            ...
            if os.path.isdir(huc_dir):
                ...
                self.all_files.extend(patch_files)
        
        logger.info(f"Found {len(self.all_files)} potential patch files")
        
        # Keep track of validated files
        self._valid_indices = []
        self._validation_attempted = set()
    ...
    def _find_next_valid_index(self, start_idx: int) -> int:
        ...
        max_search = min(100, len(self.all_files))  # Limit search to avoid infinite loops
        ...
        raise RuntimeError(f"Could not find valid file after searching {max_search} files")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if alphaearth.ndim == 2:
            alphaearth = alphaearth[None, :, :]  # Add channel dim
        elif alphaearth.ndim == 3:
            alphaearth = alphaearth.transpose(2, 0, 1)  # HWC -> CHW
        
        return {
            'alphaearth': torch.from_numpy(alphaearth),
...
def fast_evaluate_huc(model: torch.nn.Module, huc_id: str, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            alphaearth = batch['alphaearth'].to(device, non_blocking=True)
            water_mask = batch['hydro_mask'].to(device, non_blocking=True)  # Use water_mask like original
...
def fast_evaluate_all_hucs(checkpoint_path: str, huc_codes: List[str], 
    ...
    logger.info("Loading AlphaEarth model...")
    ...
    csv_filename = f"alphaearth_{run_name}_{timestamp}"
    ...
    for i, huc_id in enumerate(huc_codes, 1):
        try:
            ...
            if huc_metrics:
                result = {
                    "timestamp": datetime.now().isoformat(),
                    "model_variant": "mdmt_alphaearth_only",
                    "checkpoint_path": checkpoint_path,
                    "run_name": run_name,  
                    "huc_id": huc_id,
                    **huc_metrics
...
def main():
    parser = argparse.ArgumentParser(description="Ultra-fast AlphaEarth Evaluator")
    ...
    print("\n" + "="*80)
    print("ULTRA-FAST ALPHAEARTH EVALUATION COMPLETED (PER-HUC)")
    print("="*80)
    ...
    csv_path = os.path.join(output_dir, f"alphaearth_{run_name}_{timestamp}.csv")
