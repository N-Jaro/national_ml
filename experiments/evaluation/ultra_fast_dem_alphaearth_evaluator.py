#!/usr/bin/env python3
"""
ULTRA-FAST DEM + AlphaEarth Evaluator using lazy loading approach.
...
def setup_wandb(run_name: str, checkpoint_path: str, csv_path: str = None):
    ...
    # Initialize wandb
    wandb.init(
        project="mdmt_dem_alphaearth_evaluation",
        name=run_name,
        config=run_info,
        tags=["dem_alphaearth", "evaluation", "ultra_fast"]
    )
...
def compute_metrics_original(water_pred: torch.Tensor, water_target: torch.Tensor,
    ...
    """Compute comprehensive evaluation metrics - EXACT COPY from simple_dem_alphaearth_evaluator.py"""
    metrics = {}
...
def fast_evaluate_huc(model: torch.nn.Module, huc_id: str, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Evaluating HUC {huc_id}")):
            ...
            # Forward pass - DEM + AlphaEarth model takes two inputs
            water_pred, d8_pred = model(dem, alphaearth)
...
def fast_evaluate_all_hucs(checkpoint_path, huc_codes, device="auto", limit_batches=None, 
    ...
    # Load model
    logger.info("Loading DEM + AlphaEarth model...")
    ...
    csv_filename = f"dem_alphaearth_{run_name}_{timestamp}"
    ...
    for i, huc_id in enumerate(huc_codes, 1):
        try:
            ...
            if huc_result:
                ...
                result = {
                    'timestamp': datetime.now().isoformat(),
                    'model_variant': 'mdmt_dem_alphaearth',
                    'checkpoint_path': checkpoint_path,
                    'run_name': run_name,
                    'huc_id': huc_id,
    ...
    print("\n" + "=" * 80)
    print("ULTRA-FAST DEM + ALPHAEARTH EVALUATION COMPLETED (PER-HUC)")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Ultra-fast DEM + AlphaEarth Evaluator")
