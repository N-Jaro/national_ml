#!/usr/bin/env python3
"""
ULTRA-FAST DEM + Optical Evaluator using lazy loading approach.
...
def extract_run_name(checkpoint_path):
    ...
    if "lightning_logs" in checkpoint_path:
        ...
        for part in parts:
            if part.startswith("dem_optical_"):
                # Extract run name from folder like "dem_optical_20250930_165727_run10"
                if "_run" in part:
                    return part.split("_run")[-1]
                else:
                    return part.replace("dem_optical_", "").replace("_", "")
    
    # Fallback: use checkpoint filename
    checkpoint_name = os.path.basename(checkpoint_path)
    return checkpoint_name.replace(".ckpt", "").replace("mdmt-dem-optical-", "")
...
def setup_wandb(run_name: str, checkpoint_path: str, csv_path: str = None):
    ...
    # Initialize wandb
    wandb.init(
        project="mdmt_dem_optical_evaluation",
        name=run_name,
        config=run_info,
        tags=["dem_optical", "evaluation", "ultra_fast"]
    )
...
def compute_metrics_original(water_pred: torch.Tensor, water_target: torch.Tensor,
    ...
    """Compute comprehensive evaluation metrics - EXACT COPY from simple_dem_optical_evaluator.py"""
    metrics = {}
...
def fast_evaluate_huc(model: torch.nn.Module, huc_id: str, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Evaluating HUC {huc_id}")):
            ...
            # Forward pass - DEM + Optical model takes two inputs
            water_pred, d8_pred = model(dem, optical)
...
def fast_evaluate_all_hucs(checkpoint_path, huc_codes, device="auto", limit_batches=None, 
    ...
    # Load model
    logger.info("Loading DEM + Optical model...")
    ...
    csv_filename = f"dem_optical_{run_name}_{timestamp}"
    ...
    for i, huc_id in enumerate(huc_codes, 1):
        try:
            ...
            if huc_result:
                ...
                result = {
                    'timestamp': datetime.now().isoformat(),
                    'model_variant': 'mdmt_dem_optical',
                    'checkpoint_path': checkpoint_path,
                    'run_name': run_name,
                    'huc_id': huc_id,
    ...
    print("\n" + "=" * 80)
    print("ULTRA-FAST DEM + OPTICAL EVALUATION COMPLETED (PER-HUC)")
    print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ultra-fast DEM + Optical Evaluator")
    ...
    csv_path = os.path.join(output_dir, f"dem_optical_{run_name}_{timestamp}.csv")
