#!/usr/bin/env python3
"""
ULTRA-FAST DEM-only Evaluator using lazy loading approach.
...
from mdmt_dem_only import MultitaskModel_DEM_Only

# Add path to data module and import dataset
sys.path.append('/u/nathanj/national_ml/experiments/data')
from patchDataLoader_dem_only_fast import MultimodalPatchDataset_DEM_Only_Fast as MultimodalPatchDataset_DEM_Only
...
def setup_wandb(run_name: str, checkpoint_path: str, csv_path: str = None):
    ...
    wandb.init(
        ...
        name=run_name,
        config=run_info,
        tags=["dem_only", "evaluation", "ultra_fast"]
    )
...
def compute_metrics_dem_only(water_pred: torch.Tensor, water_target: torch.Tensor,
                            d8_pred: torch.Tensor, d8_target: torch.Tensor) -> Dict[str, float]:
    """
    Compute comprehensive evaluation metrics for DEM-only model - following AlphaEarth pattern.
    DEM-only model does output water predictions, so we compute the water metrics properly.
    """
    metrics = {}
    
    # =========================
    # WATER SEGMENTATION METRICS - Compute properly for DEM-only
    # =========================
    # Apply sigmoid to get probabilities - same as AlphaEarth
    water_prob = torch.sigmoid(water_pred)
    water_pred_binary = (water_prob > 0.5).float()
    
    # Flatten tensors for metric calculation - same as AlphaEarth
    water_pred_flat = water_pred_binary.flatten()
    water_target_flat = water_target.flatten()
    
    # Water segmentation accuracy - same as AlphaEarth
    water_accuracy = (water_pred_flat == water_target_flat).float().mean().item()
    
    # Dice coefficient (F1 for binary segmentation) - same as AlphaEarth
    intersection = (water_pred_flat * water_target_flat).sum().item()
    union = water_pred_flat.sum().item() + water_target_flat.sum().item()
    water_dice = (2.0 * intersection) / (union + 1e-8)  # Add epsilon for numerical stability
    
    # IoU (Jaccard) - same as AlphaEarth
    intersection = (water_pred_flat * water_target_flat).sum().item()
    union = (water_pred_flat + water_target_flat - water_pred_flat * water_target_flat).sum().item()
    water_iou = intersection / (union + 1e-8)
    
    # Convert to numpy for sklearn metrics - same as AlphaEarth
    water_pred_np = water_pred_flat.cpu().numpy()
    water_target_np = water_target_flat.cpu().numpy()
    
    # Precision, recall, F1 using sklearn - same as AlphaEarth
    try:
        from sklearn.metrics import precision_score, recall_score, f1_score
        water_precision = precision_score(water_target_np, water_pred_np, zero_division=0.0)
        water_recall = recall_score(water_target_np, water_pred_np, zero_division=0.0)
        water_f1 = f1_score(water_target_np, water_pred_np, zero_division=0.0)
    except Exception as e:
        logger.warning(f"Error computing water segmentation metrics: {e}")
        water_precision = water_recall = water_f1 = 0.0
    
    # =========================
    # D8 CLASSIFICATION METRICS - Same as AlphaEarth
    # =========================
    d8_pred_classes = torch.argmax(d8_pred, dim=1)  # Class indices 0-7
    
    # Map class indices to D8 values - Same mapping as AlphaEarth
    ...
    # D8 accuracy - Same as AlphaEarth (includes -2 values)
    d8_acc = (d8_pred_values == d8_target).float().mean().item()
    
    # Convert to numpy for sklearn metrics  
    d8_pred_classes_np = d8_pred_classes.flatten().cpu().numpy()
    
    # Map D8 target values back to class indices - Same mapping as AlphaEarth
    d8_value_to_class = {1: 0, 2: 1, 4: 2, 8: 3, 16: 4, 32: 5, 64: 6, 128: 7}
    ...
    # Compile all metrics - Same format as AlphaEarth
    metrics = {
        # Water segmentation metrics - Set to 0 for DEM-only
...
def fast_evaluate_huc(model: torch.nn.Module, huc_id: str, 
    ...
    """Fast evaluation for a single HUC - exactly matching AlphaEarth pattern"""
    
    logger.info(f"Evaluating HUC {huc_id}...")
    
    # Use the DEM-only dataset class - same pattern as AlphaEarth
    ...
    # Optimize batch size for GPU - same as AlphaEarth
    ...
    # Collect predictions - same pattern as AlphaEarth
    water_predictions = []
    water_targets = []
    d8_predictions = []
    d8_targets = []
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            # Move to device - same as AlphaEarth
            dem = batch['dem'].to(device, non_blocking=True)
            hydro_mask = batch['hydro_mask'].to(device, non_blocking=True) 
            flow_dir = batch['flow_dir'].to(device, non_blocking=True)
            
            # Forward pass - DEM only model
            water_pred, d8_pred = model(dem)
            
            # Collect predictions - same pattern as AlphaEarth
            water_predictions.append(water_pred)
            water_targets.append(hydro_mask)  # Use hydro_mask as water target for consistency
            d8_predictions.append(d8_pred)
            d8_targets.append(flow_dir)
    ...
    # Concatenate all predictions - exactly like AlphaEarth
    ...
    # Use the DEM-only compute_metrics function - same pattern as AlphaEarth
    metrics = compute_metrics_dem_only(water_pred_all, water_target_all, d8_pred_all, d8_target_all)
    
    # The function returns the complete metrics dict - use it directly, same as AlphaEarth
    results = metrics.copy()
    results['total_samples'] = len(water_predictions)  # Count batches like AlphaEarth
...
def fast_evaluate_all_hucs(checkpoint_path, huc_codes, device="auto", limit_batches=None, 
    ...
    """
    Evaluate all HUCs using the ultra-fast approach.
    """
    device = get_device(device)
    
    # Load model
    logger.info("Loading DEM-only model...")
    model = MultitaskModel_DEM_Only(
        n_classes_task1=1,  # Water segmentation
        n_classes_task2=8,  # Flow direction classification
        base_channels=64
    )
    ...
    csv_filename = f"dem_{run_name}_{timestamp}"
    ...
    if use_wandb:
        ...
        # Create wandb table for DEM-only (include water metrics)
        columns = ["huc_id", "water_f1", "water_iou", "d8_f1_macro", "d8_f1_weighted", "d8_accuracy", "total_samples"]
    ...
    for i, huc_id in enumerate(huc_codes, 1):
        try:
            ...
            # Get single result per HUC - AlphaEarth pattern
            ...
            if huc_result:
                # Create result in exact AlphaEarth column order
                result = {
                    'timestamp': datetime.now().isoformat(),
                    'model_variant': 'mdmt_dem_only',
                    'checkpoint_path': checkpoint_path,
                    'run_name': run_name,
                    'huc_id': huc_id,
                ...
                # Add to wandb table (DEM-only with water metrics)
                if use_wandb:
                    ...
                    # Log individual HUC metrics (include water metrics)
                    wandb.log({
                ...
                # Log HUC completion - AlphaEarth style
                logger.info(f"HUC {huc_id}: D8 F1-macro={huc_result['d8_f1_macro']:.3f}")
    ...
    if use_wandb and all_results:
        ...
        if valid_results:
            ...
            # Calculate and log summary statistics (include water metrics)
    ...
    # Summary - match AlphaEarth format exactly
    print("\n" + "=" * 80)
    print("ULTRA-FAST DEM-ONLY EVALUATION COMPLETED (PER-HUC)")
    print("=" * 80)
    ...
    if valid_results:
        # Calculate metrics including water segmentation
        ...
        # Per-HUC Results - AlphaEarth format
        print("Per-HUC Results:")
        for result in valid_results:
            # Show actual water F1 scores since DEM-only does predict water
...
def main():
    parser = argparse.ArgumentParser(description="Ultra-fast DEM-only Evaluator")
    parser.add_argument('--checkpoint', required=True, help='Path to model checkpoint')
    parser.add_argument('--huc-list', default=None, help='Optional file with HUC codes to evaluate')
    ...
    # Get HUC codes
    if args.huc_list and os.path.exists(args.huc_list):
        with open(args.huc_list, 'r') as f:
            huc_codes = [line.strip() for line in f if line.strip()]
        logger.info(f"Loaded {len(huc_codes)} HUC codes from {args.huc_list}")
    else:
        # Default test HUCs
        huc_codes = ["10020007", "14080107", "01030002"]
        logger.info(f"Using default test HUCs: {huc_codes}")
    ...
    # Run evaluation with incremental output and wandb logging
    results = fast_evaluate_all_hucs(args.checkpoint, huc_codes, device=args.device, 
