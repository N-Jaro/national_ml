#!/usr/bin/env python3
"""
Simple DEM + Optical Model Evaluator
...
def load_model_direct(checkpoint_path: str, device: str = "cpu") -> torch.nn.Module:
    ...
    try:
        # Create model instance with 6 optical channels (default for Landsat)
        model = MultimodalMultitaskModel_DEM_Optical(
            ...
            base_channels=64,
            optical_channels=6  # Landsat bands (B2,B3,B4,B5,B6,B7)
        )
        ...
        logger.info(f"Successfully loaded DEM + Optical model from {checkpoint_path}")
        return model
...
def evaluate_huc(model: torch.nn.Module, huc_id: str, test_data_path: str, 
    ...
    dataset = MultimodalPatchDataset_DEM_Optical(
        ...
        huc_codes=[huc_id],
        optical_channels=6  # Landsat bands
    )
...
def main():
    parser = argparse.ArgumentParser(description="Simple DEM + Optical Model Evaluator")
    ...
    parser.add_argument("--output-dir", type=str,
                       default="/u/nathanj/national_ml/experiments/evaluation/simple_dem_optical_results",
    ...
    csv_path = output_dir / f"simple_dem_optical_evaluation_{timestamp}.csv"
    results_df.to_csv(csv_path, index=False)
    
    # Save JSON
    json_path = output_dir / f"simple_dem_optical_evaluation_{timestamp}.json"
    results_df.to_json(json_path, orient='records', indent=2)
    
    # Print summary
    print("\n" + "="*80)
    print("DEM + OPTICAL EVALUATION COMPLETED")
    print("="*80)
