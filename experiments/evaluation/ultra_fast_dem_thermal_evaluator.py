#!/usr/bin/env python3
"""
ULTRA-FAST DEM + Thermal Evaluator using lazy loading approach.
...
def extract_run_name(checkpoint_path):
    ...
    if "lightning_logs" in checkpoint_path:
        ...
        for part in parts:
            if part.startswith("dem_thermal_"):
                # Extract run name from folder like "dem_thermal_20250930_165727_run10"
                if "_run" in part:
                    return part.split("_run")[-1]
                else:
                    return part.replace("dem_thermal_", "").replace("_", "")
    
    # Fallback: use checkpoint filename
    checkpoint_name = os.path.basename(checkpoint_path)
    return checkpoint_name.replace(".ckpt", "").replace("mdmt-dem-thermal-", "")
...
def setup_wandb(run_name: str, checkpoint_path: str, csv_path: str):
    ...
    wandb.init(
        project="mdmt_dem_thermal_evaluation",
        name=run_name,
        config={
            "checkpoint_path": checkpoint_path,
            "csv_output_path": csv_path,
            "model_variant": "mdmt_dem_thermal",
...
def compute_metrics_original(water_pred: torch.Tensor, water_target: torch.Tensor,
    ...
    """Compute comprehensive evaluation metrics - EXACT COPY from simple_dem_thermal_evaluator.py"""
    metrics = {}
...
def fast_evaluate_huc(model: torch.nn.Module, huc_id: str, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Evaluating HUC {huc_id}")):
            ...
            # Forward pass - DEM + Thermal model takes two inputs
            water_pred, d8_pred = model(dem, thermal)
...
def fast_evaluate_all_hucs(checkpoint_path, huc_codes, device="auto", limit_batches=None, 
    ...
    # Load model
    logger.info("Loading DEM + Thermal model...")
    ...
    csv_filename = f"dem_thermal_{run_name}_{timestamp}"
    ...
    print("\n" + "=" * 80)
    print("ULTRA-FAST DEM + THERMAL EVALUATION COMPLETED (PER-HUC)")
    print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ultra-fast DEM + Thermal Evaluator")
    ...
    csv_path = os.path.join(output_dir, f"dem_thermal_{run_name}_{timestamp}.csv")
