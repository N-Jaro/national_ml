...
def _parse_huc_stats(stats_json: Dict[str, Any]) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    if "optical" in stats_json:
        ...
        # Updated to handle 6 optical channels
        mean = [
...
class MultimodalPatchDataset(Dataset):
    ...
    Missing optional keys are zero-filled (in normalized space):
      - 'optical' -> zeros (H,W,6)  (B2,B3,B4,B5,B6,B7)
      - 'thermal' -> zeros (H,W)
      - 'sar'     -> zeros (H,W)

    Returns dict:
      m1 dem:       (1,H,W) float32
      m2 optical:  (6,H,W) float32 (B2,B3,B4,B5,B6,B7)
    ...
    def __init__(
        ...
        # Relaxed this assertion to allow 6 channels as per new model.
        # It's now just a check for the user.
        # assert self.optical_channels == 6, "Model expects optical with 6 channels."
        if self.optical_channels != 6:
            print(f"[WARN] optical_channels is {self.optical_channels}, but the model is configured for 6.")
