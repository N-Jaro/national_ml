...
class MultimodalPatchDataset_DEM_Only_Fast(Dataset):
    """
    ...
    def _validate_and_load_file(self, file_path: str, huc_code: str) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            with np.load(file_path) as data:
                # Required keys for DEM-only model
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for attempt in range(max_attempts):
            ...
            return {
                ...
                'flow_dir': torch.from_numpy(flow_dir).to(torch.long),
                'path': file_path
            }
...
MultimodalPatchDataset_DEM_Only = MultimodalPatchDataset_DEM_Only_Fast