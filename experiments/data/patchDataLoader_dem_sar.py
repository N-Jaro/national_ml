...
def _parse_huc_stats(stats_json: Dict[str, Any]) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    For DEM + SAR model, we only need:
      dem: elevation_mean, elevation_stdDev
      sar: VV_mean/stdDev
    """
    out = {"dem": None, "sar": None}
...
class MultimodalPatchDataset_DEM_SAR(Dataset):
    """
    ...
    Keeps sample only if required keys exist and shapes match:
      ...
      - 'sar' (2D same HxW) - now required for this dataset
            ...
            for patch_file in patch_files:
                try:
                    with np.load(patch_file) as data:
                        # Required keys for DEM + SAR model
                        ...
                        if not (dem.shape == sar.shape == hydro_mask.shape == flow_dir.shape):
                            continue
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        with np.load(file_path) as data:
            ...
            # Normalize SAR
            if huc_stats and huc_stats["sar"]:
...
def create_dataloader_dem_sar(
    ...
    **dataset_kwargs
    ...
    """Create a DataLoader for DEM + SAR model."""
