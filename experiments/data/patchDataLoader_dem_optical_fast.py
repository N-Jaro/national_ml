...
def _parse_huc_stats(stats_json: Dict[str, Any]) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    For DEM + Optical model, we need:
      dem:     elevation_mean, elevation_stdDev  
      optical: SR_B2_mean/stdDev, SR_B3_mean/stdDev, SR_B4_mean/stdDev, etc. (6 bands)
    """
...
class MultimodalPatchDataset_DEM_OPTICAL_Fast(Dataset):
    """
    ...
    def _validate_and_load_file(self, file_path: str, huc_code: str) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            with np.load(file_path) as data:
                ...
                if dem.ndim == 2 and dem.shape != base_shape:
                    self._skipped_files.add(file_path)
                    return None
                if optical.ndim == 2 and optical.shape != base_shape:
                    self._skipped_files.add(file_path)
                    return None
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for attempt in range(max_attempts):
            ...
            # Handle optical data - should be 6 channels
            if optical.ndim == 2:
                # If optical is 2D, broadcast to 6 channels: (H,W) -> (H,W,6)
                optical = np.repeat(optical[..., None], 6, axis=2)
            elif optical.ndim == 3 and optical.shape[2] != 6:
                # If wrong number of channels, pad or truncate to 6
                H, W, C = optical.shape
                if C < 6:
                    # Pad with zeros
            ...
            # Optical: (H,W,6) -> (6,H,W)
            if optical.ndim == 3:
                optical = optical.transpose(2, 0, 1)  # (H,W,C) -> (C,H,W)
            else:
                optical = optical[None, :, :]  # fallback for 2D case
...
MultimodalPatchDataset_DEM_OPTICAL = MultimodalPatchDataset_DEM_OPTICAL_Fast