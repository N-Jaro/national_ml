...
def _parse_huc_stats(stats_json: Dict[str, Any]) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    For Landsat 6B model, we need all modalities: dem, optical, thermal, sar
    """
    ...
    if "optical" in stats_json:
        ...
        # Handle 6 optical channels (B2,B3,B4,B5,B6,B7)
        mean = [
            o.get("SR_B2_mean", 0.0),
            o.get("SR_B3_mean", 0.0),
            o.get("SR_B4_mean", 0.0),
            o.get("SR_B5_mean", 0.0),
            o.get("SR_B6_mean", 0.0),
            o.get("SR_B7_mean", 0.0),
        ]
        std = [
...
class MultimodalPatchDataset_LS6B_Fast(Dataset):
    """
    ...
    def _validate_and_load_file(self, file_path: str, huc_code: str) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            with np.load(file_path) as data:
                # Required keys for Landsat 6B model (same as original loader)
                ...
                # Basic shape validation - all required keys should be 2D with same shape
                if not (dem.ndim == hydro_mask.ndim == flow_dir.ndim == 2):
                    self._skipped_files.add(file_path)
                    return None
                ...
                # Load and cache the data (including optional keys)
                result = {
                    'dem': dem.copy(),
                ...
                if 'optical' in data:
                    result['optical'] = data['optical'].copy()
                if 'thermal' in data:
                    result['thermal'] = data['thermal'].copy()
                if 'sar' in data:
                    result['sar'] = data['sar'].copy()
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for attempt in range(max_attempts):
            ...
            # Process valid data - following original loader structure exactly
            ...
            # Handle optional modalities with zero-filling if missing (like original)
            if 'optical' in data:
                optical = data['optical'].astype(np.float32)
                if optical.ndim == 2:
                    optical = np.repeat(optical[..., None], 6, axis=2)  # 6 channels
                elif optical.ndim == 3:
                    c = optical.shape[2]
                    if c < 6:
                ...
                optical = np.zeros((H, W, 6), dtype=np.float32)
                
            thermal = data['thermal'].astype(np.float32) if 'thermal' in data else np.zeros((H, W), dtype=np.float32)
            sar = data['sar'].astype(np.float32) if 'sar' in data else np.zeros((H, W), dtype=np.float32)
            ...
            if huc_stats:
                ...
                if huc_stats["optical"] is not None:
                ...
                if huc_stats["sar"] is not None:
                    sar = _zscore(sar, huc_stats["sar"]["mean"], huc_stats["sar"]["std"])
            
            # Convert to CHW format (like original)
            def _to_chw(arr):
                if arr.ndim == 2:
            ...
            m1 = _to_chw(dem)       # (1,H,W)
            m2 = _to_chw(optical)   # (6,H,W)
            m3 = _to_chw(thermal)   # (1,H,W)
            m4 = _to_chw(sar)       # (1,H,W)
...
MultimodalPatchDataset_LS6B = MultimodalPatchDataset_LS6B_Fast