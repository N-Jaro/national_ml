...
class MultimodalPatchDataset_DEM_SAR_Fast(Dataset):
    """
        ...
        try:
            with np.load(file_path) as data:
                ...
                if dem.ndim == 2 and dem.shape != base_shape:
                    self._skipped_files.add(file_path)
                    return None
                if sar.ndim == 2 and sar.shape != base_shape:
                    self._skipped_files.add(file_path)
                    return None
...
MultimodalPatchDataset_DEM_SAR = MultimodalPatchDataset_DEM_SAR_Fast