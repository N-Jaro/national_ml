...
class MultimodalPatchDataset_DEM_AlphaEarth(Dataset):
    """
    Loads <base_path>/<huc_code>/patch_*.npz with per-HUC normalization for DEM + AlphaEarth model:
    reads <base_path>/<huc_code>/normalization_stats.json when present.

    Keeps sample only if required keys exist and shapes match:
      - 'dem', 'alphaearth', 'hydro_mask', 'flow_dir'
            ...
            for patch_file in patch_files:
                try:
                    with np.load(patch_file) as data:
                        # Required keys for DEM + AlphaEarth model
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        with np.load(file_path) as data:
            ...
            # Normalize DEM only (AlphaEarth embeddings are already pre-processed)
...
def create_dataloader_dem_alphaearth(
    base_path: str,
    huc_codes: List[str],
    batch_size: int = 8,
    shuffle: bool = True,
    ...
    **dataset_kwargs
    ...
    """Create a DataLoader for DEM + AlphaEarth model."""
...
if __name__ == '__main__':
    ...
    if len(dataset) > 0:
        ...
        dataset_32 = MultimodalPatchDataset_DEM_AlphaEarth(
            base_path=base_path,
            huc_codes=huc_codes,
        ...
        dataset_128 = MultimodalPatchDataset_DEM_AlphaEarth(
            base_path=base_path,
            huc_codes=huc_codes,
