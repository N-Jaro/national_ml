#!/usr/bin/env python3
"""
CLI script for training All Modalities (without AlphaEarth) MDMT model.

This script combines DEM, Optical (6-band Landsat), Thermal (Landsat B10), 
and SAR (Sentinel-1) without AlphaEarth embeddings.

Total input channels: 1 + 6 + 1 + 1 = 9 channels (4 modalities)

Usage:
    python run_lightning_train_all_modalities.py --train_hucs "10020007,03030005" --val_hucs "16040204" --epochs 50 --batch_size 32
...
class AllModalitiesDataModule(pl.LightningDataModule):
    def __init__(
        self,
        base_path: str,
        train_hucs: List[str],
    ...
    def train_dataloader(self):
        dataset = MultimodalPatchDataset(
            ...
            optical_channels=6,  # 6-band Landsat
...
def main():
    parser = argparse.ArgumentParser(description="Train All Modalities (no AlphaEarth) MDMT model")
    ...
    parser.add_argument("--wandb_project", type=str, 
                       default="national_ml_all_modalities",
    ...
    parser.add_argument("--checkpoint_dir", type=str, 
                       default="/u/nathanj/national_ml/outputs/models/all_modalities",
    ...
    # Create data module
    data_module = AllModalitiesDataModule(
    ...
    # Create model (using existing MDMTLitModule which works with 4 modalities)
    model = MDMTLitModule(
        lr=args.lr,
    ...
    if args.experiment_name is None:
        ...
        args.experiment_name = f"all_modalities_{hucs_str}_seed{args.seed}"
    ...
    if args.wandb_mode != "disabled":
        logger = WandbLogger(
            ...
            tags=["all_modalities", "no_alphaearth", "4mod", f"seed{args.seed}"],
        )
        # Log hyperparameters
        logger.log_hyperparams({
            "model": "all_modalities_no_alphaearth",
            "total_input_channels": 9,  # 1 + 6 + 1 + 1
            "modalities": ["dem", "optical_6band", "thermal", "sar"],
            "hucs": all_hucs,
            "train_hucs": train_hucs,
            "val_hucs": val_hucs,
            **vars(args)
    ...
    print(f"Model: All Modalities without AlphaEarth (9 total channels)")
    print(f"Modalities: DEM(1) + Optical(6) + Thermal(1) + SAR(1)")
