# train_dem_optical_lightning.py
import os, sys
...
class MDMT_DEM_Optical_LitModule(pl.LightningModule):
    def __init__(
        self,
        lr: float = 1e-3,
        optimizer: str = "AdamW",
        ...
        # DEM + Optical Model
        self.model = MultimodalMultitaskModel_DEM_Optical(
            n_classes_task1=1,
            n_classes_task2=8,
    ...
    def forward(self, dem, optical):
        return self.model(dem, optical)

    def _step(self, batch, stage: str):
        dem, optical = batch["dem"], batch["optical"]
        B = dem.size(0)
        ...
        # Forward
        out_water, out_d8 = self(dem, optical)                     # (B,1,H,W), (B,8,H,W)
    ...
    def _log_val_visuals(self, batch):
        ...
        for i in range(max_samples):
            ...
            # Use first 3 channels of optical for RGB display
            opt_rgb = _to_uint8_rgb(_minmax(optical[i][:3]))     # (H,W,3)
