# train_dem_alphaearth_lightning.py
import os, sys
...
def _to_uint8_rgb(img01: torch.Tensor) -> torch.Tensor:
    ...
    if img01.size(0) == 1:
        ...
        # For AlphaEarth embeddings, use first 3 channels for visualization
        img01 = img01[:3]
...
class MDMT_DEM_AlphaEarth_LitModule(pl.LightningModule):
    def __init__(
        self,
        lr: float = 1e-3,
        optimizer: str = "AdamW",
        ...
        self.model = MultimodalMultitaskModel_DEM_AlphaEarth(
            n_classes_task1=1,
            n_classes_task2=8,
    ...
    def _step(self, batch, stage: str):
        ...
        # Forward
        out_water, out_d8 = self(dem, alphaearth)                  # (B,1,H,W), (B,8,H,W)
    ...
    def _log_val_visuals(self, batch):
        ...
        B = dem.size(0)
        max_samples = min(B, 6)  # log up to 6 samples
        images = []

        for i in range(max_samples):
            # Inputs: DEM as grayscale and AlphaEarth first 3 channels as RGB-like
            dem_rgb = _to_uint8_rgb(_minmax(dem[i]))             # (H,W,3)
            ae_rgb = _to_uint8_rgb(_minmax(alphaearth[i][:3]))   # (H,W,3)
            
            # Additional AlphaEarth visualization: average across all channels
            ...
            # Create a 2x4 panel: [DEM | AE RGB | AE Avg | Water GT] / [Water Pred | D8 GT | D8 Pred | blank]
            top_row  = torch.cat([dem_rgb, ae_rgb, ae_avg_rgb, water_gt_rgb], dim=1)      # concat width
            bot_row  = torch.cat([water_pr_rgb, d8_gt_rgb, d8_pr_rgb, torch.zeros_like(dem_rgb)], dim=1)
            panel    = torch.cat([top_row, bot_row], dim=0)                               # concat height

            images.append(wandb.Image(panel.numpy(),
                                    caption=f"epoch {self.current_epoch} | sample {i} | DEM | AE RGB | AE Avg | Water GT | Water Pred | D8 GT | D8 Pred | blank"))
