...
from train_alphaearth_only_lightning import MDMT_AlphaEarth_Only_LitModule
from data_module_alphaearth_only import PatchDataModule_AlphaEarth_Only
...
def main():
    ...
    # --- W&B flags ---
    parser.add_argument("--wandb_project", type=str, default="mdmt-hydro-alphaearth-only")
    ...
    # Model
    lit = MDMT_AlphaEarth_Only_LitModule(
        lr=args.lr,
    ...
    ckpt = ModelCheckpoint(
        ...
        filename="mdmt-alphaearth-only-{epoch:02d}-{val_loss:.4f}",
        monitor="val_loss",
        mode="min",
        save_top_k=3
    )
