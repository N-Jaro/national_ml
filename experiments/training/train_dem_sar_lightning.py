...
class MDMT_DEM_SAR_LitModule(pl.LightningModule):
    def __init__(
        self,
        lr: float = 1e-3,
        optimizer: str = "AdamW",
        ...
        # DEM + SAR Model
        self.model = MultimodalMultitaskModel_DEM_SAR(
            n_classes_task1=1,
            n_classes_task2=8,
    ...
    def forward(self, dem, sar):
        return self.model(dem, sar)

    def _step(self, batch, stage: str):
        dem, sar = batch["dem"], batch["sar"]
        B = dem.size(0)
        ...
        # Forward
        out_water, out_d8 = self(dem, sar)                         # (B,1,H,W), (B,8,H,W)
    ...
    def _log_val_visuals(self, batch):
        ...
        for i in range(max_samples):
            ...
            # Create a simple 2x3 panel: [SAR | DEM | Water GT] / [Water Pred | D8 GT | D8 Pred]
            top_row  = torch.cat([sar_rgb[:,:,:3], dem_rgb, water_gt_rgb], dim=1)      # concat width
