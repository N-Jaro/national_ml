#!/bin/bash
#SBATCH --job-name=test_alphaearth_only_1huc
...
#SBATCH --output=/u/nathanj/national_ml/experiments/training/slurm_logs/test_alphaearth_only_1huc_%j.out
#SBATCH --error=/u/nathanj/national_ml/experiments/training/slurm_logs/test_alphaearth_only_1huc_%j.err
...
echo "=== Single HUC Test: AlphaEarth Only ==="
...
echo "Architecture: AlphaEarth Only (64 channels)"
echo ""
...
python run_lightning_train_alphaearth_only.py \
    ...
    --wandb_run "alphaearth_only_1huc_test_$(date +%Y%m%d_%H%M%S)"

echo "=== AlphaEarth Only 1HUC test completed ==="