#!/bin/bash
#SBATCH --job-name=test_dem_optical_1huc
...
#SBATCH --output=/u/nathanj/national_ml/experiments/training/slurm_logs/test_dem_optical_1huc_%j.out
#SBATCH --error=/u/nathanj/national_ml/experiments/training/slurm_logs/test_dem_optical_1huc_%j.err
...
echo "=== Single HUC Test: DEM + Optical ==="
...
echo "Architecture: DEM (1) + Optical (6) = 7 channels"
echo ""
...
python run_lightning_train_dem_optical.py \
    ...
    --precision 16 \
    --optical_channels 6 \
    --wandb_run "dem_optical_1huc_test_$(date +%Y%m%d_%H%M%S)"

echo "=== DEM + Optical 1HUC test completed ==="