#!/bin/bash
#SBATCH --job-name=test_dem_only_1huc
...
#SBATCH --output=/u/nathanj/national_ml/experiments/training/slurm_logs/test_dem_only_1huc_%j.out
#SBATCH --error=/u/nathanj/national_ml/experiments/training/slurm_logs/test_dem_only_1huc_%j.err
...
echo "Architecture: DEM Only (1 channel)"
echo ""
...
python run_lightning_train_dem_only.py \
    ...
    --lr 1e-5 \
    --precision 16 \
    --wandb_run "dem_only_1huc_test_$(date +%Y%m%d_%H%M%S)"

echo "=== DEM Only 1HUC test completed ==="