# train_alphaearth_only_lightning.py
import os, sys
...
class MDMT_AlphaEarth_Only_LitModule(pl.LightningModule):
    def __init__(
        self,
        lr: float = 1e-3,
        optimizer: str = "AdamW",
        ...
        self.model = MultitaskModel_AlphaEarth_Only(
            n_classes_task1=1,
            n_classes_task2=8,
    ...
    def _step(self, batch, stage: str):
        ...
        # Forward
        out_water, out_d8 = self(alphaearth)                       # (B,1,H,W), (B,8,H,W)
    ...
    def _log_val_visuals(self, batch):
        ...
        for i in range(max_samples):
            # Inputs: Use first 3 channels of AlphaEarth for RGB-like display
            ...
            # Create a 2x3 panel: [AE RGB | AE Avg | Water GT] / [Water Pred | D8 GT | D8 Pred]
            top_row  = torch.cat([ae_rgb, ae_avg_rgb, water_gt_rgb], dim=1)      # concat width
            bot_row  = torch.cat([water_pr_rgb, d8_gt_rgb, d8_pr_rgb], dim=1)
            panel    = torch.cat([top_row, bot_row], dim=0)                      # concat height

            images.append(wandb.Image(panel.numpy(),
                                    caption=f"epoch {self.current_epoch} | sample {i} | AE RGB | AE Avg | Water GT | Water Pred | D8 GT | D8 Pred"))
