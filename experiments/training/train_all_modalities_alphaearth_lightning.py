# train_all_modalities_alphaearth_lightning.py
import os, sys
...
def _to_uint8_rgb(img01: torch.Tensor) -> torch.Tensor:
    ...
    if img01.size(0) == 1:
        ...
        # For multi-channel data, use first 3 channels for visualization
        img01 = img01[:3]
    img01 = (img01.clamp(0,1) * 255.0).byte()
    return img01.permute(1,2,0).cpu()  # (H,W,3)

def _create_optical_rgb(optical: torch.Tensor) -> torch.Tensor:
    """Create RGB visualization from Landsat optical bands (B2-B7).
    Use B4(red), B3(green), B2(blue) for natural color composite."""
    # optical: (6,H,W) - bands B2,B3,B4,B5,B6,B7
    # Extract RGB: B4(idx=2), B3(idx=1), B2(idx=0)
    if optical.size(0) >= 3:
        rgb = torch.stack([optical[2], optical[1], optical[0]], dim=0)  # R,G,B
        return _to_uint8_rgb(_minmax(rgb))
    else:
        # Fallback to grayscale if not enough bands
        return _to_uint8_rgb(_minmax(optical[0:1]))
...
class MDMT_All_Modalities_AlphaEarth_LitModule(pl.LightningModule):
    def __init__(
        self,
        lr: float = 1e-3,
        optimizer: str = "AdamW",
        ...
        # All Modalities + AlphaEarth Model (fixed 64 channels)
        self.model = MultimodalMultitaskModel_All_Modalities_AlphaEarth(
            n_classes_task1=1,
            n_classes_task2=8,
    ...
    def forward(self, dem, optical, thermal, sar, alphaearth):
        return self.model(dem, optical, thermal, sar, alphaearth)
    ...
    def _step(self, batch, stage: str):
        ...
        # Forward
        out_water, out_d8 = self(dem, optical, thermal, sar, alphaearth)  # (B,1,H,W), (B,8,H,W)
    ...
    def _log_val_visuals(self, batch):
        ...
        B = dem.size(0)
        max_samples = min(B, 4)  # log up to 4 samples (reduced due to more modalities)
        images = []

        for i in range(max_samples):
            # Inputs: All modalities
            dem_rgb = _to_uint8_rgb(_minmax(dem[i]))                 # (H,W,3)
            optical_rgb = _create_optical_rgb(optical[i])            # (H,W,3) - natural color
            thermal_rgb = _to_uint8_rgb(_minmax(thermal[i]))         # (H,W,3)
            sar_rgb = _to_uint8_rgb(_minmax(sar[i]))                 # (H,W,3)
            
            # AlphaEarth visualizations: first 3 channels as RGB-like and average
            ae_rgb = _to_uint8_rgb(_minmax(alphaearth[i][:3]))       # (H,W,3)
            ae_avg = alphaearth[i].mean(dim=0, keepdim=True)         # (1,H,W)
            ae_avg_rgb = _to_uint8_rgb(_minmax(ae_avg))              # (H,W,3)
            ...
            # Create a 2x5 panel: [DEM | Optical | Thermal | SAR | AE RGB] / [AE Avg | Water GT | Water Pred | D8 GT | D8 Pred]
            top_row  = torch.cat([dem_rgb, optical_rgb, thermal_rgb, sar_rgb, ae_rgb], dim=1)           # concat width
            bot_row  = torch.cat([ae_avg_rgb, water_gt_rgb, water_pr_rgb, d8_gt_rgb, d8_pr_rgb], dim=1) # concat width
            panel    = torch.cat([top_row, bot_row], dim=0)                                             # concat height

            images.append(wandb.Image(panel.numpy(),
                                    caption=f"epoch {self.current_epoch} | sample {i} | DEM | Optical | Thermal | SAR | AE RGB | AE Avg | Water GT | Water Pred | D8 GT | D8 Pred"))
