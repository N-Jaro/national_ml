# train_dem_thermal_lightning.py
import os, sys
...
class MDMT_DEM_Thermal_LitModule(pl.LightningModule):
    def __init__(
        self,
        lr: float = 1e-3,
        optimizer: str = "AdamW",
        ...
        # Model - DEM + Thermal variant
        self.model = MultimodalMultitaskModel_DEM_Thermal(
            n_classes_task1=1,
            n_classes_task2=8,
    ...
    def forward(self, dem, thermal):
        return self.model(dem, thermal)
    
    def _step(self, batch, stage: str):
        dem, thermal = batch["dem"], batch["thermal"]
        B = dem.size(0)
        ...
        # Forward
        out_water, out_d8 = self(dem, thermal)                     # (B,1,H,W), (B,8,H,W)
    ...
    def configure_optimizers(self):
        ...
        # allow choice of Adam or AdamW via hparams
    ...
    def _log_val_visuals(self, batch):
        ...
        for i in range(max_samples):
            ...
            # Create a simple 2x3 panel: [DEM | Thermal | Water GT] / [Water Pred | D8 GT | D8 Pred]
            top_row  = torch.cat([dem_rgb, thermal_rgb, water_gt_rgb], dim=1)      # concat width
