...
#-----------------------------------------------------------------------------
# Helper script to submit Clay foundation model array jobs
#-----------------------------------------------------------------------------

echo "🚀 Clay Foundation Model Array Submission"
echo "=========================================="

# Check if we're in the right directory
if [[ ! -f "configs/clay_config.yaml" ]]; then
    echo "❌ Error: Please run this script from the clay experiment directory"
    echo "Expected to find: configs/clay_config.yaml"
    exit 1
fi
...
sed -i "s/#SBATCH --array=.*/#SBATCH --array=1-${NUM_RUNS}%${CONCURRENT}/" submit_train_clay_array.sh

echo "🎯 Submitting Clay array job..."

# Submit the job
JOB_ID=$(sbatch --parsable submit_train_clay_array.sh)
...
if [[ $? -eq 0 ]]; then
    ...
    echo ""
    echo "📁 Check logs:"
    echo "   ls slurm_logs/clay-9ch-array_${JOB_ID}_*.{out,err}"
    echo ""
    echo "📊 Monitor on WandB:"
    echo "   https://wandb.ai/[your-entity]/clay_water_segmentation"
else
