...
Handles DEM + optical + thermal + SAR data for Clay vs Prithvi comparison.
Following the same pattern as Prithvi with proper normalization.
"""
...
def _parse_huc_stats(stats_json: Dict[str, Any]) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    if "optical" in stats_json:
        ...
        # Handle 6 optical channels (Landsat bands B2-B7)
        mean = [
            o.get("SR_B2_mean", 0.0),   # Blue
            o.get("SR_B3_mean", 0.0),   # Green
            o.get("SR_B4_mean", 0.0),   # Red
            o.get("SR_B5_mean", 0.0),   # NIR
            o.get("SR_B6_mean", 0.0),   # SWIR1
            o.get("SR_B7_mean", 0.0),   # SWIR2
        ]
        std = [
            o.get("SR_B2_stdDev", 1.0),
            o.get("SR_B3_stdDev", 1.0),
            o.get("SR_B4_stdDev", 1.0),
            o.get("SR_B5_stdDev", 1.0),
            o.get("SR_B6_stdDev", 1.0),
            o.get("SR_B7_stdDev", 1.0),
        ]
        out["optical"] = {
            "mean": np.array(mean, dtype=np.float32).reshape(1, 1, len(mean)),
            "std":  np.array(std,  dtype=np.float32).reshape(1, 1, len(std)),
        }
...
class FourModalPatchDatasetAdapter:
    ...
    def __init__(self, base_path: str, huc_codes: List[str], config: dict):
        ...
        self.stats_filename = "normalization_stats.json"
        ...
        # Load normalization stats for all HUCs
        self._load_all_huc_stats()
    ...
    def _load_all_huc_stats(self):
        """Load normalization stats for all HUCs."""
        for huc in self.huc_codes:
            self._load_huc_norm(huc)
    
    def _load_huc_norm(self, huc: str) -> Optional[Dict[str, Dict[str, np.ndarray]]]:
        """Load normalization stats for a specific HUC."""
        if huc in self.huc_norm:
            return self.huc_norm[huc]
        
        stats_path = self.base_path / huc / self.stats_filename
        if stats_path.exists():
            try:
                with open(stats_path, "r") as f:
                    stats_json = json.load(f)
                norm = _parse_huc_stats(stats_json)
                self.huc_norm[huc] = norm
                logger.info(f"Loaded normalization stats for HUC {huc}")
                return norm
            except Exception as e:
                logger.warning(f"Failed to read stats for HUC {huc}: {e}")
                self.huc_norm[huc] = None
                return None
        else:
            logger.warning(f"No normalization stats found for HUC {huc} at {stats_path}")
            self.huc_norm[huc] = None
            return None
...
class FourModalPatchDataset(Dataset):
    ...
    Compatible with Clay foundation model via TerraTorch.
    Follows the same normalization pattern as Prithvi for consistency.
    """
    
    def __init__(self, 
                 patch_metadata: pd.DataFrame, 
                 config: dict,
                 huc_norm: Dict[str, Optional[Dict[str, Dict[str, np.ndarray]]]],
                 is_training: bool = True):
        self.patch_metadata = patch_metadata
        self.config = config
        self.huc_norm = huc_norm
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        try:
            with np.load(patch_path) as npz:
                ...
                # Convert to tensor format expected by Clay multimodal wrapper
                ...
                # Clay model expects 256x256 input, but our patches are 224x224
                ...
                if current_h != 256 or current_w != 256:
                    ...
                    zoom_h = 256 / current_h
                    zoom_w = 256 / current_w
                    # Resize each channel separately
                    resized_channels = []
                    for c in range(channels):
                        resized_channel = zoom(multi_modal_image[:, :, c], (zoom_h, zoom_w), order=1)  # bilinear
                        resized_channels.append(resized_channel)
                    multi_modal_image = np.stack(resized_channels, axis=2)  # (256, 256, 9)
                
                # Convert to (C, H, W) format for PyTorch
                multi_modal_image = np.transpose(multi_modal_image, (2, 0, 1))  # (9, 256, 256)
                
                # Resize mask to match Clay's expected input size (256x256)
                if hydro_mask.shape[0] != 256 or hydro_mask.shape[1] != 256:
                    from scipy.ndimage import zoom
                    zoom_h = 256 / hydro_mask.shape[0]
                    zoom_w = 256 / hydro_mask.shape[1]
                    hydro_mask = zoom(hydro_mask, (zoom_h, zoom_w), order=0)  # nearest neighbor for masks
                
                # Prepare sample in TerraTorch format compatible with Clay multimodal wrapper
                # Note: Clay wrapper handles 9-channel input and adapts to Clay's 6-channel expectation
                sample = {
                    'image': torch.from_numpy(multi_modal_image).float(),  # (9, 256, 256)
                    'mask': torch.from_numpy(hydro_mask).float(),          # (256, 256)
    ...
    def _apply_normalization(self, dem, optical, thermal, sar, huc):
        """Apply per-HUC z-score normalization using stats file - same as Prithvi."""
        ...
        if norm is not None:
            ...
            # Fallback: simple standardization if no stats available (same as Prithvi)
...
class FourModalDataModule(pl.LightningDataModule):
    """
    PyTorch Lightning DataModule for 4-modal Clay foundation model training.
    Follows the same pattern as Prithvi DataModule for consistency.
    """
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            ...
            self.train_dataset = FourModalPatchDataset(
                ...
                self.config,
                huc_norm=self.adapter.huc_norm,
                is_training=True
            )
...
def test_four_modal_dataset():
    ...
    try:
        ...
        print("✅ Clay 4-Modal Dataset Test Results:")
        ...
        print(f"   Clay Compatibility: ✅ (Standard 4D tensor format)")
        
        # Verify channel composition
        image_sample = batch['image'][0]  # First sample
        print(f"   Channel value ranges (after normalization):")
        ...
        print(f"   Normalization: Per-HUC z-score (same as Prithvi)")
        
        return True
        
    except Exception as e:
        print(f"❌ Clay 4-Modal Dataset Test Failed: {e}")
        return False
