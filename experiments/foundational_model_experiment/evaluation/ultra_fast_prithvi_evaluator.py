#!/usr/bin/env python3
"""
ULTRA-FAST Prithvi Foundation Model Evaluator using lazy loading approach.
...
# Add path to Prithvi module
sys.path.append('/u/nathanj/national_ml/experiments/foundational_model_experiment/prithvi')

# Import Prithvi model and data module
from training.train_prithvi import PrithviFoundationModel
from data.four_modal_dataset_adapter import FourModalDataModule

# Add path to common data module
sys.path.append(str(project_root / "experiments" / "data"))
...
def setup_wandb(run_name: str, checkpoint_path: str, csv_path: str = None):
    ...
    # Initialize wandb
    wandb.init(
        project="foundational_prithvi_evaluation",
        name=f"prithvi_{run_name}_evaluation",
        config=run_info,
        tags=["evaluation", "prithvi", "foundation_model", "ultra_fast"]
    )
...
# Use the proper Prithvi data loader from the foundational experiments
    
    def _validate_file(self, file_path: str) -> bool:
        """Quick validation - check if file has required keys"""
        if file_path in self._validation_attempted:
            return False
            
        self._validation_attempted.add(file_path)
        
        try:
            with np.load(file_path, mmap_mode='r') as data:
                # Require DEM as essential input modality + evaluation targets
                required_keys = {'dem', 'hydro_mask', 'flow_dir'}
                return required_keys.issubset(set(data.keys()))
        except:
            return False
    
    def _find_next_valid_index(self, start_idx: int) -> int:
        """Find next valid file starting from start_idx"""
        max_search = min(500, len(self.all_files))  # Increased search to handle more corrupt files
        
        for i in range(max_search):
            idx = (start_idx + i) % len(self.all_files)
            file_path = self.all_files[idx]
            
            if self._validate_file(file_path):
                return idx
        
        # If no valid files found, log warning and return first index anyway
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"Could not find valid file after searching {max_search} files, using index {start_idx % len(self.all_files)}")
        return start_idx % len(self.all_files)
    
    def __len__(self) -> int:
        return len(self.all_files)
    
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        # Find a valid file starting from this index
        valid_idx = self._find_next_valid_index(idx)
        file_path = self.all_files[valid_idx]
        
        # Load the data with graceful handling of missing modalities
        try:
            with np.load(file_path) as data:
                # Required: DEM (input) + evaluation targets
                dem = data['dem'].astype(np.float32)
                hydro_mask = data['hydro_mask'].astype(np.float32)  # Ground truth for water segmentation
                flow_dir = data['flow_dir'].astype(np.int64)        # Ground truth for flow direction
                
                # Optional input modalities - fill with zeros if missing, using same shape as dem
                if 'optical' in data:
                    optical = data['optical'].astype(np.float32)  # 6 channels
                else:
                    # Create 6-channel optical with same spatial dimensions as dem
                    if dem.ndim == 2:
                        optical = np.zeros((dem.shape[0], dem.shape[1], 6), dtype=np.float32)
                    else:
                        optical = np.zeros((dem.shape[1], dem.shape[2], 6), dtype=np.float32)
                    
                if 'thermal' in data:
                    thermal = data['thermal'].astype(np.float32)
                else:
                    # Use same spatial shape as dem
                    spatial_shape = dem.shape[-2:] if dem.ndim == 3 else dem.shape
                    thermal = np.zeros(spatial_shape, dtype=np.float32)
                ...
                if 'sar' in data:
                    ...
                    # Use same spatial shape as dem
                    spatial_shape = dem.shape[-2:] if dem.ndim == 3 else dem.shape
                    sar = np.zeros(spatial_shape, dtype=np.float32)
                    
        except Exception as e:
            # If loading fails completely, create dummy data with correct shapes
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Failed to load {file_path}: {e}, creating dummy data")
            dem = np.zeros((224, 224), dtype=np.float32)
            optical = np.zeros((224, 224, 6), dtype=np.float32)
            thermal = np.zeros((224, 224), dtype=np.float32)
            sar = np.zeros((224, 224), dtype=np.float32)
            hydro_mask = np.zeros((224, 224), dtype=np.float32)
            flow_dir = np.zeros((224, 224), dtype=np.int64)
        
        # Handle DEM dimensions
        if dem.ndim == 2:
            dem = dem[None, :, :]  # Add channel dim: (H,W) -> (1,H,W)
        elif dem.ndim == 3 and dem.shape[0] != 1:
            dem = dem.transpose(2, 0, 1)  # HWC -> CHW
        
        # Handle optical dimensions (6 channels)
        if optical.ndim == 3 and optical.shape[2] == 6:
            optical = optical.transpose(2, 0, 1)  # HWC -> CHW
        elif optical.ndim == 2:
            optical = optical[None, :, :]  # Single channel case
        
        # Handle thermal dimensions
        if thermal.ndim == 2:
            thermal = thermal[None, :, :]  # Add channel dim
        elif thermal.ndim == 3 and thermal.shape[2] == 1:
            thermal = thermal.transpose(2, 0, 1)  # HWC -> CHW
        
        # Handle SAR dimensions
        if sar.ndim == 2:
            sar = sar[None, :, :]  # Add channel dim
        elif sar.ndim == 3 and sar.shape[2] == 1:
            sar = sar.transpose(2, 0, 1)  # HWC -> CHW
        
        # Concatenate all modalities: DEM(1) + Optical(6) + Thermal(1) + SAR(1) = 9 channels
        combined_input = np.concatenate([dem, optical, thermal, sar], axis=0)
        
        return {
            'image': torch.from_numpy(combined_input),  # 9-channel input for foundation model
...
def evaluate_single_huc(model: torch.nn.Module, huc_code: str, base_path: str, 
    ...
    try:
        ...
        from data.four_modal_dataset_adapter import FourModalPatchDatasetAdapter, FourModalPatchDataset
...
def main():
    parser = argparse.ArgumentParser(description='Ultra-fast Prithvi Foundation Model Evaluator')
    parser.add_argument('--checkpoint', type=str, required=True, 
                       help='Path to trained Prithvi model checkpoint')
    ...
    csv_filename = f"prithvi_{args.run_name}_{timestamp}.csv"
    csv_path = os.path.join(args.output_dir, csv_filename)
    
    logger.info(f"Starting Prithvi evaluation with checkpoint: {args.checkpoint}")
    ...
    try:
        ...
        model = PrithviFoundationModel.load_from_checkpoint(args.checkpoint)
        model = model.to(device)
        model.eval()
        logger.info(f"Successfully loaded Prithvi model with {sum(p.numel() for p in model.parameters()):,} parameters")
    except Exception as e:
    ...
    for i, huc_code in enumerate(test_hucs):
        try:
            ...
            result_record = {
                ...
                'model_variant': 'prithvi_foundation',
    ...
    if results and args.wandb_mode != 'disabled':
        ...
        log_summary_to_wandb(results_df, 'prithvi_foundation', args.run_name)
