#!/bin/bash
# SLURM Job Array for Clay Foundation Model Evaluation
# Runs 5 different Clay checkpoints concurrently as separate array tasks
...
#SBATCH --job-name=clay_array_eval

# Specify the output and error files for each array task
#SBATCH --output=slurm_logs/clay_array_%A_%a.out
#SBATCH --error=slurm_logs/clay_array_%A_%a.err

# Clay Foundation Model Array Evaluation
# Each array task evaluates one run (checkpoint) on all 67 HUCs

echo "Starting Clay Array Task ${SLURM_ARRAY_TASK_ID}/5 at $(date)"
...
RUN_NAME="run5" # Temporary fix to always use run4 for testing
...
if [ $EVAL_EXIT_CODE -eq 0 ]; then
    ...
    echo "Results saved to: ultra_fast_results/clay_${RUN_NAME}_*.csv"
else
