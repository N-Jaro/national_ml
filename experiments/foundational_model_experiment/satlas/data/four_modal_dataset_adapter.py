...
class FourModalPatchDatasetAdapter:
    ...
    def _split_patches(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Reproducible random split (SATLAS version - no random_seed requirement)
        random.seed(42)  # Fixed seed for SATLAS
        random.shuffle(patch_ids)
        
        # Apply train ratio (SATLAS uses train_ratio instead of split_ratio)
        train_ratio = self.config['data'].get('train_ratio', 0.8)
        split_idx = int(len(patch_ids) * train_ratio)
