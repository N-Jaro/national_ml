...
Individual training script for SatLas foundation model
"""
...
class SatlasFoundationModel(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        logger.info(f"Initializing SatLas foundation model")
        ...
        logger.info(f"SatLas model initialized with {self.count_parameters():,} parameters")
        
        # Debug counter for logging
        self._debug_step = 0
    
    def _build_model(self):
        """Build SatLas model with multimodal wrapper for 9-channel input."""
        try:
            from satlas_multimodal_wrapper import SatlasMultimodalWrapper
            
            # Try TerraTorch approach first, fall back to wrapper if needed
            try:
                logger.info("Attempting to build SatLas model through TerraTorch...")
                factory = EncoderDecoderFactory()
                
                # Use the Sentinel-2 bands specified in config
                model_bands = self.config["model"]["model_bands"]
                ...
                model_config = {
                    ...
                    "backbone_kwargs": {"model_bands": model_bands}
                }
                
                model = factory.build_model(**model_config)
                
                # Test the model to ensure it works
                test_input = torch.randn(1, 9, 224, 224)
                with torch.no_grad():
                    try:
                        output = model(test_input)
                        logger.info("Successfully built SatLas model through TerraTorch")
                        logger.info(f"Model bands: {model_bands}")
                        return model
                    except Exception as e:
                        logger.warning(f"TerraTorch model forward test failed: {e}")
                        raise e
                        
            except Exception as terratorch_error:
                logger.warning(f"TerraTorch approach failed: {terratorch_error}")
                logger.info("Falling back to SatLas multimodal wrapper...")
                
                # Use the multimodal wrapper approach
                wrapper = SatlasMultimodalWrapper(self.config)
                logger.info("Built SatLas model using multimodal wrapper")
                logger.info("SatLas multimodal input: DEM + 6×Optical + Thermal + SAR = 9 channels")
                
                return wrapper
            
        except Exception as e:
            logger.error(f"Error building SatLas model: {e}")
            raise
    ...
    def configure_optimizers(self):
        # Ensure all optimizer parameters are proper types
        lr = float(self.config["training"]["learning_rate"])
        weight_decay = float(self.config["training"]["weight_decay"])
        beta1 = float(self.config["training"]["beta1"])
        beta2 = float(self.config["training"]["beta2"])
...
def main():
    parser = argparse.ArgumentParser(description="SatLas Foundation Model Experiment Training")
    parser.add_argument("--config", default="configs/satlas_config.yaml", 
    ...
    # Train
    logger.info("Starting SatLas training...")
    trainer.fit(model, data_module)
    
    logger.info("SatLas training completed!")
