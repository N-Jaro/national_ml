...
#-----------------------------------------------------------------------------
# Helper script to submit SatLas foundation model array jobs
#-----------------------------------------------------------------------------

echo "🚀 SatLas Foundation Model Array Submission"
echo "============================================"

# Check if we're in the right directory
if [[ ! -f "configs/satlas_config.yaml" ]]; then
    echo "❌ Error: Please run this script from the satlas experiment directory"
    echo "Expected to find: configs/satlas_config.yaml"
    exit 1
fi
...
sed -i "s/#SBATCH --array=.*/#SBATCH --array=1-${NUM_RUNS}%${CONCURRENT}/" submit_train_satlas_array.sh

echo "🎯 Submitting SatLas array job..."

# Submit the job
JOB_ID=$(sbatch --parsable submit_train_satlas_array.sh)
...
if [[ $? -eq 0 ]]; then
    ...
    echo ""
    echo "📁 Check logs:"
    echo "   ls slurm_logs/satlas-9ch-array_${JOB_ID}_*.{out,err}"
    echo ""
    echo "📊 Monitor on WandB:"
    echo "   https://wandb.ai/[your-entity]/satlas_water_segmentation"
else
